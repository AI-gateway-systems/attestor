...
|---|---|---|---|
| OPS-25 empty Alertmanager receivers | closed / live-proof-only | `ops/observability/alertmanager/alertmanager.yml` now uses secret-file webhook receiver configs; `scripts/render/render-alertmanager-config.mjs` continues to fail fast in production mode; `tests/ops-sweep-03-observability-remediation.test.ts` locks the contract. | Live paging still requires real receiver URLs and an alert delivery probe. |
| OPS-26 literal Prometheus metrics token | closed | `ops/observability/prometheus/prometheus.yml` uses `authorization.credentials_file`; `scripts/render/render-observability-credentials.ts` emits `prometheus-metrics-token`; compose mounts that file read-only. | Token rotation and secret backend proof remain live ops work. |

...
| Ops and infrastructure | runtime readiness; operational boundedness | Ops Sweep 01/02/03/04 remediation, HA manifests, observability alerting/token hardening, PITR/Redis recovery hardening, collector securityContext, and live proof register entries. | Cloud/IAM/KMS/network/storage observability proof, alert delivery, backend auth/storage, PITR/Redis recovery proof, and budget telemetry proof. | Continue ops sweep with autoscaling, profiles, recording rules, and remaining provider overlays. |

...
| `OPS-SWEEP-18` | Signing surface deep audit across Ed25519 primitives, strict canonical JSON, certificate verification, PKI trust-chain verification, keyless signer CA boundary, PKI-bound certificate verification, and the GCP KMS adapter contract. Primitive signing layer is repo-proven for native Ed25519, SPKI-DER fingerprints, fail-closed verification, 6-step chain verification, and 4-binding PKI trust checks; KMS runtime activation remains an accepted Phase 2 limitation. | `1c6092332eb9bf98fc1c4eb4d5b9c3178a955db6` | `partial` | `docs/audit/ops-sweep-18-signing-surface-deep-audit.md`. | Promote OPS-141 production-like keyless CA fail-closed before stronger runtime signing claims; OPS-142 remains the Phase 2 path for `External KMS runtime signing`. |

...
| In progress | 0 |
| Not started | 3 |
| Current posture | Step 07 is complete: the `gke-production-rehearsal` target now has a Redis/BullMQ async recovery rehearsal that consumes the Step 05 substrate readiness summary and Step 06 consequence behavior summary, requires `production-shared`, `REDIS_URL`, and the `queue-redis` substrate, then exercises Redis `noeviction` posture, BullMQ retry/stalled guardrails, worker drain/restart, retry recovery, terminal failure DLQ visibility, fail-quick unavailable-Redis submission, and Redis-backed tenant coordination visibility. Passing this command is target-bound technical evidence, not a blanket production guarantee. The next work is Step 08: rehearse backup, restore, and DR. |
...
Implement Step 08: rehearse backup, restore, and DR. Run control-plane snapshot, PostgreSQL backup/PITR drill, Redis durability check, restore validation, and post-restore readiness/admission checks against a replacement target.

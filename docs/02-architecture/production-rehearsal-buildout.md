...
| In progress | 0 |
| Not started | 2 |
| Current posture | Step 08 is complete: the `gke-production-rehearsal` target now has a backup/restore/DR rehearsal that consumes Step 05, Step 06, Step 07, and operator PostgreSQL PITR evidence before running the existing control-plane backup/restore commands against separated source and replacement PostgreSQL targets. It validates replacement release-authority, control-plane, and billing PostgreSQL state, checks source and replacement Redis durability posture, calls replacement API/worker readiness endpoints, and runs a post-restore consequence-admission allow/block probe. Passing this command is target-bound DR rehearsal evidence, not automated cross-region failover or a blanket production guarantee. The next work is Step 09: rehearse observability, alerting, and operator runbooks. |
...
Implement Step 09: rehearse observability, alerting, and operator runbooks. Prove telemetry receiver reachability, alert routing, dashboard/runtime truth visibility, and runbook stop conditions for the named target.

...
| In progress | 0 |
| Not started | 5 |
| Current posture | Step 05 is complete: the `gke-production-rehearsal` target now has a fail-closed substrate readiness probe that checks required environment inputs, shared PostgreSQL/Redis readiness, External Secrets, Gateway, HTTPRoute, cert-manager Certificate conditions, DNS, API/worker readiness endpoints, and observability receivers. Passing this probe is target-specific evidence, not a blanket production guarantee. The next work is Step 06: rehearse core fail-closed consequence behavior against the named target. |
...
Implement Step 06: rehearse core fail-closed consequence behavior. Exercise admitted, blocked, review/hold, token issuance, token introspection, revocation, replay/use-count exhaustion, evidence-pack export, reviewer queue claim, and downstream gate behavior against the named target environment.

...
| Consequence admission core | `src/consequence-admission/index.ts`, `src/consequence-admission/contracts.ts`, `src/consequence-admission/engine.ts`, `src/consequence-admission/generic-engine.ts` | Exposes the shared admission package facade and owns the request/response contract, decisions, checks, proof refs, generic mode ladder, retry guidance, feedback, canonical digest, and descriptor through split internal modules. |

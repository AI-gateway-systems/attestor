...
- Remaining limitation or no-go condition: This is not a live AWS, Google Cloud, Azure, HSM, or confidential-compute signer adapter and does not activate external release-token signing. It prevents production-shared route readiness overclaim until a real provider adapter, sign/verify probe, rotation plan, compromise response, durable introspection/replay backend, and deployed customer PEP evidence exist.

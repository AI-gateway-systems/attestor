...
function testTrackerMarksStep04CompleteWithoutRenumbering(): void {
  ...
  includes(tracker, '| Completed | 10 |', 'Production rehearsal target: tracker marks ten steps complete');
  includes(tracker, '| Not started | 0 |', 'Production rehearsal target: tracker leaves no frozen steps not started');
  includes(
    tracker,
  ...
  includes(
    ...
    'Production rehearsal target: immediate next step advances to target-run execution',
  );
}

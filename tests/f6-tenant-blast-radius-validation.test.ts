...
includes(validation, 'No F6 item is considered fixed by this validation pass', 'F6 validation: scope is not overclaimed');
...
includes(tracker, 'Remaining F6 queue after validation slice: 7 planned', 'Tracker: F6 remaining count is explicit');
includes(tracker, 'F6 validation and tracker sync', 'Tracker: F6 queue names the current validation slice');

includes(tenantIsolation, 'const tenantKeys = new Map', 'Repo evidence: env tenant key cache is module-local');
...
ok(!releaseObjectModel.includes('readonly tenant_id'), 'Repo evidence: release token claims do not yet include tenant_id');

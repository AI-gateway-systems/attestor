...
export const GOLDEN_EXTERNAL_COMMUNICATION_RUNTIME_SMOKE_VERSION =
  'attestor.golden-external-communication-runtime-smoke.v1';

export interface GoldenExternalCommunicationRuntimeSmokeScenarioResult {
...
export interface GoldenExternalCommunicationRuntimeSmokeResult {
  readonly version: typeof GOLDEN_EXTERNAL_COMMUNICATION_RUNTIME_SMOKE_VERSION;
  readonly step: 'E03';
  ...
  readonly scenarioResults: readonly GoldenExternalCommunicationRuntimeSmokeScenarioResult[];
...
export interface GoldenExternalCommunicationRuntimeSmokeDescriptor {
  readonly version: typeof GOLDEN_EXTERNAL_COMMUNICATION_RUNTIME_SMOKE_VERSION;
  readonly step: 'E03';
  readonly sourceFixtureSuiteVersion: typeof GOLDEN_EXTERNAL_COMMUNICATION_SHADOW_FIXTURES_VERSION;
  readonly sourcePolicyFoundryProjectionVersion:
  ...
  readonly shadowRuntimeFixtureReplaySmokeVersion:
    typeof SHADOW_RUNTIME_FIXTURE_REPLAY_SMOKE_VERSION;
  readonly scenarioCount: 8;
  readonly runsE01FixturesThroughR02ToR07: true;
  readonly executionMode: 'shadow-only';
  readonly fixtureOnly: true;
  readonly deterministicReplay: true;
  readonly noTargetSystemCall: true;
...
function minute(index: number, seconds: number): string {
  return `2026-05-26T10:${String(index).padStart(2, '0')}:${String(seconds).padStart(2, '0')}.000Z`;
}

function assertProjectionMatchesSuite(
  suite: GoldenExternalCommunicationShadowFixtureSuite,
  projection: GoldenExternalCommunicationPolicyFoundryProjection,
): void {
  if (projection.sourceFixtureSuiteDigest !== suite.digest) {
    throw new Error(
      'Golden external communication runtime smoke projection must be derived from the fixture suite.',
    );
  }
  if (projection.sourceFixtureCount !== suite.fixtureCount) {
    throw new Error(
      'Golden external communication runtime smoke projection must retain the full fixture count.',
    );
  }
  if (
    ...
    throw new Error(
      'Golden external communication runtime smoke requires a review-only projection.',
    );
  }
}
...
function runScenarioSmoke(
  ...
  if (!fixture) {
    throw new Error('Golden external communication runtime smoke requires a fixture for each scenario.');
  }
  return runShadowRuntimeFixtureReplaySmoke({
    fixtureId: fixture.fixtureId,
    fixtureRefDigest: fixture.digest,
    event: fixture.event,
    sourcePartitionDigest: digestFor('golden-external-communication-source-partition', suite.digest),
    traceContextDigest: digestFor('golden-external-communication-trace-context', {
      scenario: fixture.scenario,
      eventDigest: fixture.event.digest,
    }),
    sourceHistoryRefDigest: digestFor('golden-external-communication-source-history', suite.digest),
    ...
    workerRefDigest: digestFor('golden-external-communication-worker', 'fixture-only-worker'),
    dispatcherRunDigest: digestFor('golden-external-communication-dispatcher', {
      scenario: fixture.scenario,
      sequence: index,
    }),
    observerRefDigest: digestFor(
      'golden-external-communication-observer',
      'fixture-only-observer',
    ),
    evaluatorRefDigest: digestFor(
      'golden-external-communication-evaluator',
      'fixture-only-evaluator',
    ),
    scopeDigest: digestFor(
      'golden-external-communication-scope',
      'shadow-only-runtime-smoke',
    ),
  });
}
...
): GoldenExternalCommunicationRuntimeSmokeScenarioResult {
  return Object.freeze({
    scenario: smoke.fixtureId.replace(/^golden-external-communication:/u, ''),
...
export function runGoldenExternalCommunicationRuntimeSmoke(
  suite: GoldenExternalCommunicationShadowFixtureSuite =
    createGoldenExternalCommunicationShadowFixtureSuite(),
  projection: GoldenExternalCommunicationPolicyFoundryProjection =
    createGoldenExternalCommunicationPolicyFoundryProjection(suite),
): GoldenExternalCommunicationRuntimeSmokeResult {
  ...
  const payload = {
    version: GOLDEN_EXTERNAL_COMMUNICATION_RUNTIME_SMOKE_VERSION,
    step: 'E03',
...
export function goldenExternalCommunicationRuntimeSmokeDescriptor():
  ...
  return Object.freeze({
    ...
    shadowRuntimeFixtureReplaySmokeVersion: SHADOW_RUNTIME_FIXTURE_REPLAY_SMOKE_VERSION,
    scenarioCount: 8,
    runsE01FixturesThroughR02ToR07: true,
    executionMode: 'shadow-only',
    fixtureOnly: true,
    deterministicReplay: true,
    noTargetSystemCall: true,
